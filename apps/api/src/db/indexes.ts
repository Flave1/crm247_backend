...
export async function ensureIndexes(db: Db): Promise<void> {
  await Promise.all([
    ...
    db.collection("contacts").createIndex({ domainId: 1, email: 1 }, { unique: true }),
    db.collection("contacts").createIndex({ domainId: 1, updatedAt: -1 }),
    db.collection("outbound_messages").createIndex({ trackingId: 1 }, { unique: true }),
    db.collection("outbound_messages").createIndex({ domainId: 1, createdAt: -1 }),
    db.collection("email_events").createIndex({ contactId: 1, timestamp: -1 }),
    db.collection("email_events").createIndex({ trackingId: 1, timestamp: -1 }),
    db.collection("email_events").createIndex({ domainId: 1, timestamp: -1 }),
